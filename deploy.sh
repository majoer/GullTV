...
ssh ${REMOTE_USER}@${REMOTE_HOST} "sudo mv /tmp/${APP_NAME}.service /etc/systemd/system/"
...
ssh ${REMOTE_USER}@${REMOTE_HOST} "sudo systemctl restart ${APP_NAME}"

ssh ${REMOTE_USER}@${REMOTE_HOST} "journalctl -u ${APP_NAME}.service -f"